...
object Hooker {
    ...
    inline fun <T> hook(
        ...
        crossinline filter: ((HookAdapter<T>) -> Boolean),
        crossinline consumer: (HookAdapter<T>) -> Unit,
    ): XC_MethodHook.Unhook = XposedBridge.hookMethod(member, newMethodHook(stage, consumer, filter))
    ...
    fun <T> hook(
        ...
        stage: HookStage,
        consumer: (HookAdapter<T>) -> Unit,
    ): XC_MethodHook.Unhook = hook(member, stage, { true }, consumer)
